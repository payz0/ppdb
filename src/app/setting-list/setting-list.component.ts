...
// nanti di lanjut
  cekSemuaSiswa(local){
    ...
    this._data.tampilData('admin').subscribe((data)=>{
       ...
       // console.log(gotData)
       this.datas = gotData.sort((a,b)=>{return  b.konfirm - a.konfirm || b.total - a.total || a.jarak - b.jarak });
      //  this.datas = this.datas.sort((a,b)=>{return  b.total - a.total});
      //  this.datas = this.datas.sort((a,b)=>{return  b.konfirm - a.konfirm});
      })
...
delete(){
	for(let i = 0; this.siswas.length > i ; i++){
		this._data.hapusData('siswa',this.siswas[i]).subscribe(
			(data)=>{
				this._data.pesan('siswa telah di hapus');
				this.daftarSiswa();
			},(err)=>{
				this._data.pesan('ada kesalahan database');
			})
      ...
      data.sort((a,b)=>{return a.jauh - b.jauh}).forEach((val,key)=>{
        if(zona[0] != null){
          if(key == zona[0]){
            ...
            // return val.point;
              this._data.updateData({konfirm:true,total:val.point,raport:0,prestasi:0,zonasi:val.point,ket:null},'siswa',id).subscribe(
                 (datas)=>{
          ...
          this.nilai_zonasi = 0;
          // return 0;
            this._data.updateData({konfirm:true,total:0,raport:0,prestasi:0,zonasi:0,ket:null},'siswa',id).subscribe(
               (datas)=>{
                 this._data.pesan("Data sudah di konfirm");
                 this.daftarSiswa();
               },(err)=>{
